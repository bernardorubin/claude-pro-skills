...
- The project path is already registered in `~/.config/claude-pro-skills/vaults.json` (offer to overwrite explicitly)
...
Open in [Obsidian](https://obsidian.md). Entry point: `wiki/index.md`. The vault auto-updates during Claude sessions via the `vault-keeper` skill in claude-pro-skills.
...
Update `~/.config/claude-pro-skills/vaults.json`. Create the file + parent dir if missing.

```bash
mkdir -p ~/.config/claude-pro-skills

if [ ! -f ~/.config/claude-pro-skills/vaults.json ]; then
  echo '{"vaults":{}}' > ~/.config/claude-pro-skills/vaults.json
fi

# Add the entry (idempotent — overwrites if key exists):
jq --arg p "{project-path}" --arg v "{vault-path}" \
  '.vaults[$p] = $v' \
  ~/.config/claude-pro-skills/vaults.json > ~/.config/claude-pro-skills/vaults.json.tmp \
  && mv ~/.config/claude-pro-skills/vaults.json.tmp ~/.config/claude-pro-skills/vaults.json
```
...
- `/save-session-to-worklog` — vault-aware; routes worklogs into `{vault}/raw/work-logs/<user-slug>/` (per-user subfolder, derived from `git config user.email` or `$CLAUDE_PRO_SKILLS_VAULT_USER`) for any project registered in the vault registry
