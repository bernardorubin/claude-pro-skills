...
Before marking it ready, run the project's designated self-review on your own changes — default
`/pr-review`, but check the repo's CLAUDE.md for a specialized reviewer (some repos define a
custom review agent). Address its findings before pushing the PR.
...
Push the follow-up commits. Update the PR description if the change set materially shifted.
