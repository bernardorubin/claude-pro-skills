...
`cut-release` is the per-**release** half; [[shipit]] is the per-**ticket**
half. They don't overlap: `shipit` ends at a review-ready PR; `cut-release`
begins after those PRs are merged, when you're turning a pile of merged work into
one versioned build. A week of `shipit` runs → one `cut-release`.

Like `shipit`, this is a conductor, not the orchestra. Lean on:
